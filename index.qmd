...
# HVAD GØR VI HER?
df1 = pd.read_json('/Users/jeppe/Library/CloudStorage/OneDrive-AalborgUniversitet/01_work/01_undervisning/02_sds1/03_data/right-wing-sites/DKSamling_20230627.json')

df2 = pd.read_json('/Users/jeppe/Library/CloudStorage/OneDrive-AalborgUniversitet/01_work/01_undervisning/02_sds1/03_data/right-wing-sites/trykkefrihed_20220824.json')
...
for cluster_num in range(best_k):
    ...
    mask = (df['cluster'] == cluster_num).to_numpy()
...
```{python}
# Definer entities der skal reklassificeres (skal matche exact case)
reclassify_map = {
    'hong kong': 'LOC',
    'Hong Kong': 'LOC',  
    'usa': 'LOC',
...
# Lav en mask for hvilke rækker der skal ændres
mask = entities_df['text'].isin(reclassify_map.keys())
entities_df.loc[mask, 'label'] = entities_df.loc[mask, 'text'].map(reclassify_map)

# Filtrer PER entities
top_persons = entities_df[entities_df['label'] == 'PER']['text'].value_counts().head(10)
print(top_persons)
```
