...
2. Gennemgange af meningsfulde eksempler på en SDS analyse 

# Sprogmodeller (slides fra L10)

## Analyse-eksempel 1

> Komparativ (diskurs/tekst-)analyse af ekstremistiske fora
...
* **Data**: 
    ...
    * *Muligheder*: Forstå sproglige markører i radikaliserede kontekster. **Hvad ellers?** 
...
print(f"\nDKsamling: {len(df[df['source']=='mainstream_media'])}")
print(f"\nTrykkefrihed: {len(df[df['source']=='alternative_blogs'])}")
print(f"\nManglende værdier:\n{df.isnull().sum()}")
...
print(f"Min: {df['n_tokens'].min()}, Max: {df['n_tokens'].max()}")
```
...
* Det betyder vi MISTER 95.5% af informationen!
* Dette er normalt for højdimensionelle tekstdata:
    * Tekstdata er meget 'spredt' i mange dimensioner
    * Det er svært at fange al information i kun 2D
    * Vores plot er derfor en APPROKSIMATION
...
```{python}
# Simpel version uden forvirrende tærskler
var_df = pd.DataFrame({
...
Diskuter hvad kan forbedre analysen og hvilken vide analyse har bidraget med om de to tekstkilder?
